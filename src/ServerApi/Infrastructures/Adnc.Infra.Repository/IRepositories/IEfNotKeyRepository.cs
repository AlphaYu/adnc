...
public interface IEfNotKeyRepository<TEntity> : IEfNotKeyBaseRepository<TEntity>
   where TEntity : EfEntityNotKey
{
    ...
    IQueryable<TrdEntity> GetAll<TrdEntity>(bool writeDb = false, bool noTracking = true) where TrdEntity : EfEntityNotKey;
    ...
    /// </summary>
    /// <param name="whereExpression">查询条件</param>
    /// <param name="isForceDel">是否强制删除</param>
    /// <param name="cancellationToken"><see cref="CancellationToken"/></param>
    /// <returns></returns>
    Task<int> DeleteRangeAsync(Expression<Func<TEntity, bool>> whereExpression, bool isForceDel = false, CancellationToken cancellationToken = default);
}