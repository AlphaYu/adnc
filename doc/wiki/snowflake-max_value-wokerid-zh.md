## 前言
&ensp; &ensp; 生成主键的方法有多种，如自增 Id、Guid、Redis 的 incr、雪花算法等。各种方法的优劣已有大量资料，本文不再赘述。Adnc 采用基于 [Yitter](https://github.com/yitter/IdGenerator) 的雪花算法生成 Id，以兼顾分布式环境下的唯一性与高性能。
## Yitter雪花算法介绍
...
? 当发生系统时间回拨时，算法采用历史时序的预留序号生成新的 ID。

? 回拨生成的 ID 序号，默认靠前，也可调整为靠后。
...
| WorkerId          | 机器Id，最重要参数，无默认值，必须 全局唯一，必须 <b>程序设定</b>。 |
