...
5. 服务间通信通常有三种方式：
   - HTTP（Refit）：见 `doc/wiki/service-http-call-zh.md`
   - gRPC：见 `doc/wiki/service-grpc-call-zh.md`
   - 事件（CAP）：见 `doc/wiki/service-event-call-zh.md`
...
用于“加载配置”。支持本地文件和 Consul KV（详见 `doc/wiki/config-center-zh.md`）：
...
用于“服务注册与发现”。支持 `Direct/Consul/CoreDns`（配置说明见 `doc/wiki/appsettings-zh.md`，使用介绍见 `doc/wiki/registry-center-zh.md`）：
...
- **查询/校验**：用 HTTP 或 gRPC，同步拿结果，链路尽量短  
  - HTTP：`doc/wiki/service-http-call-zh.md`
  - gRPC：`doc/wiki/service-grpc-call-zh.md`
- **跨服务写协作/最终一致性**：优先用事件，便于解耦，也能减少调用链  
  - CAP：`doc/wiki/service-event-call-zh.md`

### 3.4 认证授权

ADNC 在“服务互调”场景下提供了更工程化的做法：既支持 Basic，也支持 Bearer 透传，减少业务代码对 Token 处理的重复劳动（见 `doc/wiki/claims-based-authentication-zh.md`）。

### 3.5 缓存、分布式锁、布隆过滤器

缓存相关能力（包括防雪崩、击穿、穿透这些常见问题的处理方式）集中在一篇文档里，适合直接照着落地（见 `doc/wiki/cache-redis-distributedlock-bloomfilter-zh.md`）。

### 3.6 数据访问与事务（EF Core + UnitOfWork）

仓储、工作单元、本地事务/分布式事务、原生 SQL 等，都有对应的 Demo 和文档（可以先从 `doc/wiki/efcore-*.md` 系列开始）。

### 3.7 可观测性与运维友好（日志 / 链路 / 健康检查）

在微服务里，“能看见、好排查”往往比“能写代码”更重要。ADNC 已经把很多基础能力做成统一接入点：

- 链路追踪：SkyAPM（SkyWalking .NET Agent），开启方式见 `doc/wiki/skyapm-tracing-zh.md`。
- 健康检查：服务默认会暴露健康检查端点，也可以配合 Consul 做健康探测（注册中心部分也会用到）。
- 日志与审计：项目有统一的日志配置和输出方式（可以从 `doc/wiki/appsettings-zh.md` 的 Logging 节点开始理解）。

---

## 4. 本地如何跑起来（最少需要改哪些东西）

如果你只想先“跑起来看看”，按 `doc/wiki/quickstart-zh.md` 的顺序就可以。核心就三件事：
...
1. `doc/wiki/quickstart-zh.md`：先跑起来
2. `doc/wiki/appsettings-zh.md`：弄清楚关键配置节点
3. `doc/wiki/config-center-zh.md`：配置中心（如果你有多环境/统一下发的需求）
4. `doc/wiki/registry-center-zh.md`：注册中心（如果你要做真实的服务发现和负载均衡）
5. `doc/wiki/service-http-call-zh.md` / `doc/wiki/service-grpc-call-zh.md`：同步调用怎么写、鉴权怎么带、地址怎么配
6. `doc/wiki/service-event-call-zh.md`：事件驱动怎么做幂等、怎么处理事务与重试
7. `doc/wiki/feature-dev-guide-zh.md` + `doc/wiki/api-dev-guide-zh.md` + `doc/wiki/service-dev-guide.md`：跟着规范新增一个 CRUD
8. 需要排查性能或调用链时读 `doc/wiki/skyapm-tracing-zh.md`
9. 需要部署到服务器或容器时读 `doc/wiki/quickly-docker-deploy-zh.md`
